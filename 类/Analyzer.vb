...
Public Class Analyzer
    ...
    Public Function ReadConc() As Boolean
        ...
        Select Case sendCount
            Case 1
                'Try
                '    ''速越仪器
                '    'a = master.ReadHoldingRegisters(10, 211, 4)
                '    '_tchConc = ModbusUtility.GetSingle(a(0), a(1))
                '    '_ch4Conc = ModbusUtility.GetSingle(a(2), a(3))

                '    '/*自研甲烷分析仪*/
                '    a = master.ReadInputRegisters(1, 8, 2)
                '    _ch4Conc = a(0) / 100
                '    _tchConc = a(1) / 100
                ...
                Catch ex As Modbus.SlaveException
                    ' Modbus从站返回的特定异常（最重要的错误信息）
                    Dim errorMsg As String =
                        $"Slave异常! 功能码:{ex.FunctionCode}, 错误码:{ex.SlaveExceptionCode} - {GetSlaveExceptionMeaning(ex.SlaveExceptionCode)}"
                    WriteLogToText(Now.ToString("yyyyMMdd"), "alarm", errorMsg)
                    sendCount += 1
                    Return False

                Catch ex As System.TimeoutException
                    ' 超时异常：从站未响应
                    Dim errorMsg As String = "读取超时：从站设备未响应。请检查连接、电源、串口参数和从站地址。"
                    WriteLogToText(Now.ToString("yyyyMMdd"), "alarm", errorMsg)
                    sendCount += 1
                    Return False
                ...
                Catch ex As System.IO.IOException
                    ' IO异常：串口通信错误
                    Dim errorMsg As String = $"IO异常：串口通信错误 - {ex.Message}"
                    WriteLogToText(Now.ToString("yyyyMMdd"), "alarm", errorMsg)
                    sendCount += 1
                    Return False
                ...
                Catch ex As System.ArgumentException
                    ' 参数异常：地址或数量错误
                    Dim errorMsg As String = $"参数异常：起始地址或寄存器数量无效 - {ex.Message}"
                    WriteLogToText(Now.ToString("yyyyMMdd"), "alarm", errorMsg)
                    sendCount += 1
                    Return False
                ...
                Catch ex As System.UnauthorizedAccessException
                    ' 串口访问权限异常
                    Dim errorMsg As String = "串口访问被拒绝：可能端口已被其他程序占用或权限不足。"
                    WriteLogToText(Now.ToString("yyyyMMdd"), "alarm", errorMsg)
                    sendCount += 1
                    Return False
                ...
                Catch ex As System.InvalidOperationException
                    ' 无效操作异常：串口未打开等
                    Dim errorMsg As String = $"无效操作：{ex.Message}"
                    WriteLogToText(Now.ToString("yyyyMMdd"), "alarm", errorMsg)
                    sendCount += 1
                    Return False
                ...
                Catch ex As Exception
                    ' 其他未预料到的异常
                    Dim errorMsg As String = $"未知异常: {ex.GetType().Name} - {ex.Message}"
                    WriteLogToText(Now.ToString("yyyyMMdd"), "alarm", errorMsg)
                    sendCount += 1
                    Return False
            ...
            Case 2
                ...
                Try
                    ...
                    If oldo2value <> _o2value Then
                        ...
                        master.WriteSingleRegister(1, &H44, _o2value * 100)
    ...
    ' 辅助函数：获取Slave异常码的含义
    Private Function GetSlaveExceptionMeaning(code As Byte) As String
        Select Case code
            Case 1 : Return "非法功能（设备不支持功能码04）"
            Case 2 : Return "非法数据地址（地址错误或超出范围）"
            Case 3 : Return "非法数据值（寄存器数量无效等）"
            Case 4 : Return "从站设备故障"
            Case 5 : Return "确认（已接收命令但需要较长时间执行）"
            Case 6 : Return "从站设备忙"
            Case 8 : Return "存储奇偶性错误"
            Case 10 : Return "网关路径不可用"
            Case 11 : Return "网关目标设备响应失败"
            Case Else : Return $"未知错误码: {code}"
        End Select
    End Function
